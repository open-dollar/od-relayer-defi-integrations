...
import {GmxDeposit} from '@libraries/gmx/GmxDeposit.sol';
import {GmxMarket} from '@libraries/gmx/GmxMarket.sol';
import {GmxMarketPoolValueInfo} from '@libraries/gmx/GmxMarketPoolValueInfo.sol';
import {GmxPrice} from '@libraries/gmx/GmxPrice.sol';
import {GmxWithdrawal} from '@libraries/gmx/GmxWithdrawal.sol';
...
interface IGmxReader {
  ...
  error MaxRefPriceDeviationExceeded(address token, uint256 price, uint256 refPrice, uint256 maxRefPriceDeviationFactor);
  ...
  function getDeposit(IGmxDataStore _dataStore, bytes32 _key) external view returns (GmxDeposit.DepositProps memory);

  function getWithdrawal(
    IGmxDataStore _dataStore,
    bytes32 _key
  ) external view returns (GmxWithdrawal.WithdrawalProps memory);
